  ...
  return (
    <div className={cn("flex flex-col items-center justify-center w-full relative", className)}>
      ...
      <div className={cn(
        ...
        <div 
          className="font-display font-extrabold text-foreground leading-[0.8] tracking-[0.01em] py-2"
          style={{ fontSize: 'clamp(100px, 22vw, 220px)' }}
        >
          {weightChars.map((char, i) => (
            <SlotDigit key={`w-${i}`} digit={char} index={i} isStable={isStable} />
          ))}
        </div>
      ...
      <div className="text-center transition-all duration-300">
        <div 
          className="font-display font-extrabold text-foreground leading-[0.85] tracking-[0.005em] py-1"
          style={{ fontSize: 'clamp(60px, 14vw, 140px)' }}
        >
          {currencySymbol}
          {priceChars.map((char, i) => (
            <SlotDigit key={`p-${i}`} digit={char} index={i} isStable={isStable} />
          ))}
        </div>
        <div className="text-foreground/85 font-label text-sm tracking-[0.15em] mt-1">
